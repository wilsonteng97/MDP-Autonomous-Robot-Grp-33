...
const int MAX_SMALL_SENSOR = 999;
const int MAX_BIG_SENSOR = 1500;
...
void readFrontSensor_1() {
  double irDistance = analogRead(A3)//Front left
  frontIR1_Median.add(irDistance);
  if (frontIR1_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(frontIR1_Median.getHighest() - frontIR1_Median.getLowest()) > 400) {
...
void readFrontSensor_2() {
  double irDistance = analogRead(A1);//Middle
  frontIR2_Median.add(irDistance);
  if (frontIR2_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(frontIR2_Median.getHighest() - frontIR2_Median.getLowest()) > 400) {
...
void readFrontSensor_3() {
  double irDistance = analogRead(A0);//Front right
  frontIR3_Median.add(irDistance);
  if (frontIR3_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(frontIR3_Median.getHighest() - frontIR3_Median.getLowest()) > 400) {
...
void readRightSensor_1() {
  double irDistance = analogRead(A2);//right back
  rightIR1_Median.add(irDistance);
  if (rightIR1_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(rightIR1_Median.getHighest() - rightIR1_Median.getLowest()) > 400) {
...
void readRightSensor_2() {
  double irDistance = analogRead(A5);//right front
  rightIR2_Median.add(irDistance);
  if (rightIR2_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(rightIR2_Median.getHighest() - rightIR2_Median.getLowest()) > 400) {
...
void readLeftSensor_1() {
  double irDistance = analogRead(A4);//Long Range
  leftIR_1_Median.add(irDistance);
  if (leftIR_1_Median.getCount() >= NUM_SAMPLES_MEDIAN) {
    if (abs(leftIR_1_Median.getHighest() - leftIR_1_Median.getLowest()) > 400) {
