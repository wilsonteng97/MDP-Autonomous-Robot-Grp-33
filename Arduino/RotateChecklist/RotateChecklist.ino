...
void setup() {
  ...
  setpointInit = 80;
  setRotation = 720;
  setTickDistance = rotToTicks(setRotation);
  Serial.print("Angle is "); Serial.print(setRotation); Serial.print(" degrees, or "); Serial.print(setTickDistance); Serial.println(" ticks.");
...
void loop() {
  ...
  if ((distanceTicksL + distanceTicksR)/2 >= setTickDistance) {
    ...
    md.setSpeeds(0, 0);
    Serial.print("Angle of "); Serial.print(setRotation); Serial.println(" degrees reached.");
  }
  else if ((currTime - printTime) >= 500000) {
...
double calculateSpeedL(double rpm) {
  double speedL = 3.3079 * rpm + 24.723;
  return speedL;
}
