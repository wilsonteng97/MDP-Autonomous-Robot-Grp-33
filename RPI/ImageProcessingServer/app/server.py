...
class ImageProcessingServer:
    ...
    def start(self):
        ...
        while True:
            ...
            raw_image_path = os.path.join('C:\\\\Users\\xiaoqing\\Documents\\school stuff\\Year 3 Sem 1\\CZ3004 MDP\\image-rec-2\\images',raw_image_name)
            # save raw image
            ...
            import subprocess
            cur_dif = os.getcwd()
            os.chdir('C:\\\\Users\\xiaoqing\\Documents\\school stuff\\Year 3 Sem 1\\CZ3004 MDP\\image-rec-2')
            cmd = ['python', 'detect_object_rcnn.py', '--image', 'images/{}'.format(raw_image_name)]
            subprocess.run(cmd)

            """ # load the our fine-tuned model and label binarizer from disk
            print("[INFO] loading model and label binarizer...")
            model = load_model(pyimage_config.MODEL_PATH)
            lb = pickle.loads(open(pyimage_config.ENCODER_PATH, "rb").read())
            ...
            for (x, y, w, h) in rects[:pyimage_config.MAX_PROPOSALS_INFER]:
                # extract the region from the input image, convert it from BGR to
                # RGB channel ordering, and then resize it to the required input
                # dimensions of our trained CNN
                roi = image[y:y + h, x:x + w]
                roi = cv2.cvtColor(roi, cv2.COLOR_BGR2RGB)
                roi = cv2.resize(roi, pyimage_config.INPUT_DIMS,
            ...
            idxs = np.where(proba[:,:]>pyimage_config.MIN_PROBA)[0]
            ...
            for i in boxIdxs:
                ...
                for j in range(15):
                    if proba[i][j]>pyimage_config.MIN_PROBA:
